    ...
    return <>
        <ResumeListContext.Provider value={{ alarm, open, anchorEl, handleClick, canBeOpen, id, checkedList, resumeStateAlarm, buttonMatch }}>
            <Container>
                ...
                <BodyContainer>
                    {resumeList.map((list) => {
                        const tags = list.tech_tags.split(",")
                        tags.sort()
                        // console.log('tags',tags)
                        return<BodyRow key={list.num}>
                            <LeftWrap>
                                <Checkbox {...label}
                                    defaultChecked size="small"
                                ...
                                />
                                {/* 사진 */}
                                <Avatar alt={list.user_name} src="/static/images/avatar/1.jpg" sx={{marginLeft:'7px'}}/>
                                {/* 스펙 */}
                                <div style={{display:'flex',flexDirection:'column',marginLeft:'10px'}}>
                                    <div className="new-resume-profile" style={{cursor:'pointer',paddingBottom:'5px'}}
                                    onClick={()=>{navi(`/resume/detail/${list.user_id}/${list.resume_idx}`)}}>
                                        {list.user_name} <span style={{borderLeft:'2px solid lightgray',paddingLeft:'3px',fontSize:'13px',color:'rgb(71 71 71)'}}>{list.job_position}</span> <span style={{borderLeft:'2px solid lightgray',paddingLeft:'3px',fontSize:'13px',color:'rgb(71 71 71)'}}>{list.title}</span>
                                    </div>
                                    <div>
                                        {tags.map((data,id)=>(<Chip label={data} variant="outlined" sx={{fontSize:'10px',height:'17px',color:'#666',marginRight:'3px'}}/>))}
                                    </div>
                                </div>
                            </LeftWrap>
