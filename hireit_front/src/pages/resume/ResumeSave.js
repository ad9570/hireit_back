...
const ResumeSave = () => {
    ...
    let updatesaveresume = `${process.env.REACT_APP_SPRING_URL}resume/updatesaveresume`;
    let callBack = `${process.env.REACT_APP_SPRING_URL}resume/savedetail?username=${user_id}&resume_idx=${resume_idx}`; //get일때만 사용가능
    ...
    const savedInfo = () => {
        axios.get(callBack).then(res => {
            ...
            setUser_addr(res.data.user_addr.split(','));
    ...
    //수정 값 보내기 
    const save = () => {
        axios.post(updatesaveresume, {
            user_id, username, name, user_hp, user_photo, addr, addr_detail, user_addr: user_addr.join(','), user_birth, user_email, user_gender, sch_name, sch_region, sch_start, sch_end, sch_major,
            ...
            tech_tags: bodyFormData, com_cv, intro, pot_link, pot_file, res_name, job_type: job_type.join(','), resume_idx, writeday
        })
            .then(res => {
                handleOpen();
            })
